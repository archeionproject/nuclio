...
const Mib = 1048576
const FunctionProcessorConfigPath = "/etc/nuclio/config/processor/processor.yaml"
	...
	if newPlatform.Config.Local.FunctionContainersHealthinessEnabled {
		...
		go func(newPlatform *Platform) {
			...
			for range uptimeTicker.C {
				newPlatform.ValidateFunctionServiceHealthiness(ctx)
			}
		}(newPlatform)
	}
...
func (p *Platform) CreateFunction(ctx context.Context, createFunctionOptions *platform.CreateFunctionOptions) (
	*platform.CreateFunctionResult, error) {
	...
	if _, err := p.QueryOPAFunctionPermissions(ctx, createFunctionOptions.FunctionConfig.Meta.Labels[common.NuclioResourceLabelKeyProjectName],
...
func (p *Platform) RedeployFunction(ctx context.Context, redeployFunctionOptions *platform.RedeployFunctionOptions) error {
	...
	// TODO: update the service with the new requirements
	/*
		// Check OPA permissions
...
func (p *Platform) GetFunctionActiveReplicaNames(ctx context.Context,
	...
	// TODO: get running tasks ??
	return []string{
		p.GetFunctionServiceName(function.GetConfig()),
	}, nil
}
...
func (p *Platform) GetFunctionReplicaContainers(ctx context.Context, functionConfig *functionconfig.Config, replicaName string) ([]string, error) {
	// / TODO: get all task ??
	return []string{
		p.GetFunctionServiceName(functionConfig),
	}, nil
}
...
func (p *Platform) GetName() string {
	return common.SwarmPlatformName
}
...
func (p *Platform) CreateFunctionEvent(ctx context.Context, createFunctionEventOptions *platform.CreateFunctionEventOptions) error {
	...
	permissionOptions.RaiseForbidden = true
	if _, err := p.QueryOPAFunctionEventPermissions(ctx, projectName,
		functionName,
...
func (p *Platform) ValidateFunctionServiceHealthiness(ctx context.Context) {
	...
	for _, namespace := range namespaces {
		...
		for _, function := range functions {
			...
			serviceName := p.GetFunctionServiceName(&functionconfig.Config{
			...
			services, err := p.DockerClient.GetServices(&dockerclient.GetServiceOptions{
				Name: serviceName,
			})
			if err != nil {
				p.Logger.WarnWithCtx(ctx, "Failed to get services by name",
					"err", err,
					"containerName", serviceName)
				continue
			}

			// if function service does not exists, mark as unhealthy
			if len(services) == 0 {
				p.Logger.WarnWithCtx(ctx, "No services were found", "functionName", functionName)

				// no running services were found for function, set function unhealthy
			...
			// check ready function to ensure its tasks are healthy
			if functionIsReady {
				if err := p.checkAndSetFunctionUnhealthy(service.ID, function); err != nil {
...
func (p *Platform) GetFunctionServiceName(functionConfig *functionconfig.Config) string {
	return fmt.Sprintf("nuclio-%s-%s",
		functionConfig.Meta.Namespace,
		functionConfig.Meta.Name)
}

func (p *Platform) GetFunctionConfigName(functionConfig *functionconfig.Config) string {
	return fmt.Sprintf("nuclio-%s-%s",
		functionConfig.Meta.Namespace,
		functionConfig.Meta.Name)
}
...
func (p *Platform) deployFunction(createFunctionOptions *platform.CreateFunctionOptions,
	...
	if !createFunctionOptions.FunctionConfig.Spec.Disable {
		...
		functionExternalHTTPPort, err = p.resolveDeployedFunctionHTTPPort(serviceID)
		if err != nil {
	...
	return &platform.CreateFunctionResult{
		...
		// TODO: temporary use service ID as container ID
...
func (p *Platform) delete(ctx context.Context, deleteFunctionOptions *platform.DeleteFunctionOptions) error {
	...
	getServiceOptions := &dockerclient.GetServiceOptions{
		Labels: map[string]string{
			"nuclio.io/platform":                      common.SwarmPlatformName,
...
func (p *Platform) resolveAndCreateFunctionMounts(
	createFunctionOptions *platform.CreateFunctionOptions) ([]dockerclient.MountPoint, []dockerclient.ConfigMount, error) {

	if err := p.prepareFunctionConfigMount(createFunctionOptions); err != nil {
		return nil, nil, errors.Wrap(err, "Failed to prepare a function's config and volumes mount")
	}
	...
	configMounts := []dockerclient.ConfigMount{
		{
			Source: p.GetFunctionConfigName(&createFunctionOptions.FunctionConfig),
			Target: FunctionProcessorConfigPath,
		},
	}
	return mountPoints, configMounts, nil
}
...
func (p *Platform) deleteOrStopFunctionService(createFunctionOptions *platform.CreateFunctionOptions) (int, error) {
	...
	if createFunctionOptions.FunctionConfig.Spec.Disable {
		if len(services) != 1 {
			return 0, errors.New("Function cannot be disabled since it has zero or more than one service")
		}

		createFunctionOptions.Logger.InfoWith("Disabling function",
			"functionName", createFunctionOptions.FunctionConfig.Meta.Name)

		// if function is disabled, stop services
		createFunctionOptions.Logger.DebugWith("Stop function container",
			"functionName", createFunctionOptions.FunctionConfig.Meta.Name,
			"serviceID", services[0].ID)
		if err := p.DockerClient.StopService(services[0].ID); err != nil {
			return 0, errors.Wrap(err, "Failed to stop function service")
		}
...
func (p *Platform) checkAndSetFunctionUnhealthy(serviceID string, function platform.Function) error {
...
func (p *Platform) waitForService(serviceID string, timeout int) error {
...
func (p *Platform) prepareFunctionConfigMount(createFunctionOptions *platform.CreateFunctionOptions) error {
	...
	if _, err := p.DockerClient.CreateConfig(p.GetFunctionConfigName(&createFunctionOptions.FunctionConfig), string(processorConfigBody)); err != nil {
		return errors.Wrapf(err, "Failed to create a config for function %s",
			createFunctionOptions.FunctionConfig.Meta.Name)
	}
...
func (p *Platform) populateFunctionInvocationStatus(functionInvocation *functionconfig.Status,
	...
	services, err := p.DockerClient.GetServices(&dockerclient.GetServiceOptions{
		ID: createFunctionResults.ContainerID,
	})

	if err != nil || len(services) == 0 {
		return errors.Wrap(err, "Failed to get service network addresses")
	}

	var addressesWithFunctionPort []string
	for _, address := range services[0].Endpoint.VirtualIPs {
		addressesWithFunctionPort = append(addressesWithFunctionPort,
			fmt.Sprintf("%s:%d", strings.Split(address.Addr, "/")[0], abstract.FunctionContainerHTTPPort))
	}
